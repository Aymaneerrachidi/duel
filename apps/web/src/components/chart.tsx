...
export function PerformanceChart({duel,snapshots,mini=false}:{duel:DuelView;snapshots:Snapshot[];mini?:boolean}){
  ...
  return <div className={`performance-chart ${mini?'mini-chart':''}`}>{!mini&&<div className="chart-toolbar"><div className="chart-legend"><span><i className="legend-a"/>{nameOf(duel.challengerProfile)}</span><span><i className="legend-b"/>{nameOf(duel.opponentProfile)}</span></div><div className="segmented" aria-label="Chart time range">{['24H','3D','ALL'].map(v=><button key={v} className={range===v?'selected':''} onClick={()=>setRange(v)}>{v}</button>)}</div></div>}
    <div className="chart-canvas"><svg viewBox={`0 0 ${w} ${h}`} role="img" aria-label="Cash-flow-adjusted return chart, normalized to 100 at start" onMouseMove={e=>{const r=e.currentTarget.getBoundingClientRect();setHover((e.clientX-r.left)/r.width*w);}} onMouseLeave={()=>setHover(null)}>
      <defs><linearGradient id={`area${id}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3558f5" stopOpacity=".1"/><stop offset="100%" stopColor="#3558f5" stopOpacity="0"/></linearGradient></defs>
      {!mini&&[0,1,2,3,4].map(i=>{const value=lower+(upper-lower)*i/4;return <g key={i}><line x1={pl} x2={w-pr} y1={y(value)} y2={y(value)} stroke="#e0e5ef" strokeDasharray="3 6"/><text x={pl-12} y={y(value)+4} textAnchor="end" fill="#626b83" fontSize="11" fontFamily="Geist Sans, sans-serif">{value>0?'+':''}{value.toFixed(0)}%</text></g>;})}
      {paths[0]&&<path d={`${paths[0]} L${x(series[0].at(-1)!.timestamp)},${h-pb} L${x(series[0][0].timestamp)},${h-pb} Z`} fill={`url(#area${id})`}/>}
      {paths.map((path,i)=><path key={i} d={path} fill="none" stroke={i===0?'#3558f5':'#d45f40'} strokeWidth={mini?2:2.5} strokeLinejoin="round" strokeLinecap="round"/>)}
      {!mini&&<><text x={pl} y={h-3} fill="#626b83" fontSize="11" fontFamily="Geist Sans, sans-serif">{new Date(minX).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</text><text x={w-pr} y={h-3} textAnchor="end" fill="#626b83" fontSize="11" fontFamily="Geist Sans, sans-serif">{duel.status==='COMPLETED'?'FINISH':'LATEST SNAPSHOT'}</text></>}
      {hover!==null&&!mini&&<line x1={hover} x2={hover} y1={pt} y2={h-pb} stroke="#a5adc0" strokeDasharray="3 3"/>}
